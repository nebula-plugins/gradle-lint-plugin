...
class DuplicateDependencyClassRuleSpec extends BaseIntegrationTestKitSpec {
    ...
    def 'dependencies with duplicate classes cause violations results in #message - custom configuration'() {
        ...
        buildFile.text = """
            ...
            dependencies {
                ${deps.collect { "smokeTest '$it'" }.join('\n')}
            }
        """
        ...
        where:
        deps                            | message
        [guava, collections]            | "$collections in configuration ':smokeTest' has 309 classes duplicated by $guava"
        [guava_transitive, collections] | "$collections in configuration ':smokeTest' has 309 classes duplicated by $guava"
        [asm, asm_asm]                  | "$asm_asm in configuration ':smokeTest' has 21 classes duplicated by $asm"
    }
     ...
     *
     * public interface HornetQUtilLogger extends BasicLogger {*      HornetQUtilLogger LOGGER = Logger.getMessageLogger(...);
     *}*/
    @Issue('52')
